...
function newPlayer(ws) {
  if (
    ...
    gameInstances[gameInstances.length - 1].players.length >= 4 ||
    gameInstances[gameInstances.length - 1].gameState !== 0
  ) {
    gameInstances.push(new GameInstance(gameInstances.length));
    console.log(
      "GI " + gameInstances[gameInstances.length - 1].id + ": Created"
    );
  }
...
function throwDice(ws) {
  ...
  if (gameInstance.currentPlayer === player.playern) {
    ...
    gameInstance.players.forEach((element) => {
      element.ws.send(
        "1-" + gameInstance.gameState + "=" + gameInstance.currentPlayer
      );
    });
  } else {
    console.log(
...
function movePawn(ws, data) {
  ...
  if (data === "0") {
    ...
    if (gameInstance.currentPlayer === currPlayer.playern) {
      let currPlace =
        gameInstance.pawns[currPlayer.playern - 1][pawnNumber - 1];
      ...
      if (currPlace === 0) {
        if (gameInstance.currentDice === 6) {
          ...
          console.log(
            "GI " +
              ...
              ": Player " +
              currPlayer.playern +
              " move pawn from home without 6 CHEAT?"
          );
        }
        //DEFAULT BEHAVIOR
      } else if (currPlace !== 0) {
        ...
        if (
          newBoardPlace > destNumbers[currPlayer.playern - 1] &&
          newBoardPlace <= destNumbers[currPlayer.playern - 1] + 7 &&
          currPlace <= destNumbers[currPlayer.playern - 1] &&
          currPlace <= 40
        ) {
          isDestSquare = true;
        ...
        if (newBoardPlace <= 40 + (currPlayer.playern - 1) * 4 + 4) {
          ...
          if (newBoardPlace > 40 + (currPlayer.playern - 1) * 4 + 4)
            newBoardPlace = 40 + (currPlayer.playern - 1) * 4 + 4;
          if (newBoardPlace > 40 && !isDestSquare) newBoardPlace -= 40;
        }
      }
      currPlayer.gameInstance.pawns[currPlayer.playern - 1][pawnNumber - 1] =
        newBoardPlace;
  ...
  for (let i = 0; i < 4; ++i) {
    if (
      gameInstance.pawns[gameInstance.currentPlayer - 1][i] <=
        40 + (gameInstance.currentPlayer - 1) * 4 ||
      gameInstance.pawns[gameInstance.currentPlayer - 1][i] >
        44 + (gameInstance.currentPlayer - 1) * 4
    )
      hasWon = false;
  }
  if (hasWon) {
    ...
    for (let i = 0; i < 4; ++i) {
      if (i !== pawnPlayer - 1) {
        for (let j = 0; j < gameInstance.pawns[i].length; ++j) {
          if (
            gameInstance.pawns[i][j] ===
            gameInstance.pawns[pawnPlayer - 1][pawnNumber - 1]
          ) {
            gameInstance.pawns[i][j] = 0;
            gameInstance.players.forEach((element) => {
              element.ws.send("3-" + (i + 1) + "=" + (j + 1) + "=0");
...
function getWaiting() {
  let waiting = 0;
  if (gameInstances[gameInstances.length - 1].players.length !== 4)
    waiting = gameInstances[gameInstances.length - 1].players.length;
  return waiting;
}
